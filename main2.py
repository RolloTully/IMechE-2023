...
time.sleep(0.5)
connection.mav.command_long_send(connection.target_system,connection.target_component, mavutil.mavlink.MAV_CMD_SET_MESSAGE_INTERVAL,0,33,2000,1,0,0,0,0)
connection.recv_match(type='COMMAND_ACK', blocking = True)
connection.mav.command_long_send(connection.target_system,connection.target_component, mavutil.mavlink.MAV_CMD_SET_MESSAGE_INTERVAL,0,83,2000,1,0,0,0,0)
connection.recv_match(type='COMMAND_ACK', blocking = True)
...
while True:
    ...
    print(connection.messages['GLOBAL_POSITION_INT'])
    sleep(0.5)
